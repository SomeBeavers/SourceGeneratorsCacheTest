...
{
    [Generator]
    public class DifferentContentSameNameDependsOnExternalFile : ISourceGenerator
    {
        ...
        public static string GenerateClassFile(string className, string csvText, CsvLoadType loadTime, bool cacheObjects)
        {
            ...
            sb.Append(@"
#nullable enable
namespace DifferentContentSameNameDependsOnExternalFile {
    using System.Collections.Generic;
...
");
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            foreach ((string name, string code) in nameCodeSequence)
            {
                // Timeout
                Thread.Sleep(20000);
                context.AddSource($"DifferentContentSameNameDependsOnExternalFile_{name}", SourceText.From(code, Encoding.UTF8));
            }
        }
